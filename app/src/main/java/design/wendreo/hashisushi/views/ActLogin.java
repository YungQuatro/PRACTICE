...
public class ActLogin extends AppCompatActivity implements View.OnClickListener {
	...
	public void addUserLogin ( String email, String senha ) {
		userAuth.createUserWithEmailAndPassword ( email, senha )
				.addOnCompleteListener ( ActLogin.this, new OnCompleteListener< AuthResult > ( ) {
					...
					public void onComplete ( @NonNull Task< AuthResult > task ) {
						if ( task.isSuccessful ( ) ) {
							msgShort ( "Почти закончили..." );
							msgShort ( "Идёт обработка!" );
							initSignup ( );
						} else {
							msgShort ( "К сожалению регистрация не была завершена :(" );
	...
	private void iniciarCadastro ( ) {
		...
		alert.setMessage ( "\nЧтобы зарегистрировать учетную запись введите свой адрес электронной почти и установите пароль" );
		...
		edtEmail.setHint ( "Введите ваш е-mail" );
		
		layoutFilds.addView ( edtPassword );
		edtPassword.setHint ( "Введите ваш пароль" );
		
		alert.setView ( layoutFilds );
		
		
		alert.setPositiveButton ( "Продолжить регистрацию", new DialogInterface.OnClickListener ( ) {
			@Override
			public void onClick ( DialogInterface dialog, int which ) {
				...
				if ( email.equals ( "" ) || password.equals ( "" ) ) {
					...
					msgShort ( "Внимание, заполните поля!" );
					
					if ( email.equals ( "" ) ) {
						msgShort ( "Введите действительный адрес почты!" );
					} else if ( password.equals ( "" ) ) {
						msgShort ( "Введите пароль!" );
					}
